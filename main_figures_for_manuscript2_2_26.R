...
geo_labels <- data.frame(
  ...
  lon = c(-118.10, -119.85, -117.16,      
          -119.70, -120.05, -120.50,      
          -119.35, -119.03, -119.65,      
          -118.52, -118.55),
  lat = c(34.15, 34.45, 32.60,           
          34.12, 33.92, 34.12,            
          34.00, 33.48, 33.18,            
          33.48, 32.78),
...
# Create combined dataset with ALL points to avoid
all_points_to_avoid <- bind_rows(
  site_coordinates %>% 
    select(Longitude, Latitude) %>%
    rename(lon = Longitude, lat = Latitude) %>%
    mutate(point_type = "study_site", point_size = 1.5),
  geo_labels %>% 
    select(lon, lat) %>%
    mutate(point_type = "label", point_size = 0.5)
)
...
fig1_main <- ggplot() +
  ...
  #ALL POINTS TO AVOID
  geom_point(
    data = all_points_to_avoid,
    aes(x = lon, y = lat, size = point_size),
    alpha = 0,
    show.legend = FALSE,
    inherit.aes = FALSE
  ) +
...
# Creating Palos Verdes Peninsula inset map
# Filter PV sites
palos_verdes_sites <- site_coordinates %>%
  filter(Longitude > -118.45 & Longitude < -118.25 &
           Latitude > 33.65 & Latitude < 33.80)

cat("\nPalos Verdes sites found:", nrow(palos_verdes_sites), "\n")
if(nrow(palos_verdes_sites) > 0) {
  print(palos_verdes_sites %>% select(Site, Longitude, Latitude))
}
...
#Combining main figure with BOTH inset maps
fig1 <- ggdraw() +
  draw_plot(fig1_main) +
  # California reference 
  draw_plot(
    inset_map_california,
    x = 0.71,
    y = 0.65,
    width = 0.15,
    height = 0.15
...
ggsave(
  filename = file.path(fig_dir, "fig1_COMPLETE_final.png"),
  plot = fig1,
  width = 10,
  height = 7,
  dpi = 600,
...
ggsave(
  filename = file.path(fig_dir, "fig1_COMPLETE_final.pdf"),
  plot = fig1,
  width = 10,
  height = 7,
  device = cairo_pdf
