...
# ============================================================================
# CREATE HISTOGRAM OF DENSITIES FOR ZOE
# ============================================================================

cat("\n==============================================\n")
cat("CREATING DENSITY DISTRIBUTION HISTOGRAMS\n")
cat("==============================================\n\n")

# Overall density histogram
overall_hist <- ggplot(gorg_data_clean %>% filter(mean_gorgonian_density > 0), 
                       aes(x = mean_gorgonian_density)) +
  geom_histogram(bins = 50, fill = "steelblue", color = "black", alpha = 0.7) +
  scale_x_log10() +
  labs(
    title = "Distribution of Gorgonian Densities (All Species Combined)",
    subtitle = "Log scale, excluding zeros",
    x = "Gorgonian Density (individuals/m²) - LOG SCALE",
    y = "Frequency"
  ) +
...
species_hists <- gorg_data_clean %>%
  filter(mean_gorgonian_density > 0) %>%
  ggplot(aes(x = mean_gorgonian_density)) +
  geom_histogram(bins = 30, fill = "steelblue", color = "black", alpha = 0.7) +
  scale_x_log10() +
  facet_wrap(~BenthicReefSpecies, scales = "free") +
  labs(
    title = "Distribution of Gorgonian Densities by Species",
    subtitle = "Log scale, excluding zeros",
    x = "Gorgonian Density (individuals/m²) - LOG SCALE",
    y = "Frequency"
  ) +
...
ggsave(
  filename = "Density_Distribution_Overall.png",
  plot = overall_hist,
  path = save_folder,
  width = 10,
  height = 6,
...
ggsave(
  filename = "Density_Distribution_By_Species.png",
  plot = species_hists,
  path = save_folder,
  width = 14,
  height = 10,
...
cat("Histograms saved for Zoe to review!\n\n")

# Print summary statistics
cat("DENSITY SUMMARY STATISTICS:\n")
cat("---------------------------\n")
density_stats <- gorg_data_clean %>%
  filter(mean_gorgonian_density > 0) %>%
  group_by(BenthicReefSpecies) %>%
  summarise(
    n = n(),
    min = min(mean_gorgonian_density),
    median = median(mean_gorgonian_density),
    mean = mean(mean_gorgonian_density),
    max = max(mean_gorgonian_density),
    sd = sd(mean_gorgonian_density)
  ) %>%
  arrange(desc(median))
...
# ============================================================================
# UPDATED HEATMAP FUNCTION - NO POINTS AT ALL
# ============================================================================

create_species_heatmap <- function(species_name, data, global_max = NULL, 
                                   save_path = NULL, color_option = "viridis") {
  ...
  #Creating main plot - NO POINTS AT ALL
  main_plot <- ggplot() +
    ...
    #Interpolating density surface ONLY - NO POINTS
    ...
    #For VERY sparse species (like L. alba), show points instead of interpolation
    ...
    #Coloring scale - EXPERIMENTING WITH DIFFERENT GRADIENTS
    {if(color_option == "viridis") {
    ...
    #Merging fill and size legends for L. alba
    {if(is_sparse) {
  ...
  if(!is.null(save_path)) {
    ggsave(
      filename = paste0(gsub(" ", "_", species_name), "_Heatmap_", color_option, ".png"), 
      plot = final_plot,
      path = save_path, 
      width = 8, 
      height = 7, 
    ...
    #Also saving as PDF
    ggsave(
      filename = paste0(gsub(" ", "_", species_name), "_Heatmap_", color_option, ".pdf"), 
      plot = final_plot,
      path = save_path, 
      width = 8, 
      height = 7,
...
# ============================================================================
# GENERATE HEATMAPS WITH DIFFERENT COLOR GRADIENTS
# ============================================================================
...
#Generating heatmaps for each species
species_list <- unique(gorg_data_clean$BenthicReefSpecies)

# Testing different color gradients
color_options <- c("viridis", "magma", "turbo", "rocket", "mako")

cat("Generating heatmaps with different color gradients:\n")
for(color_opt in color_options) {
  cat("\n  Testing color gradient:", color_opt, "\n")
  
  for(species in species_list) {
    cat("    -", species, "\n")
    plot <- create_species_heatmap(
      species, 
      gorg_data_clean, 
...
# ============================================================================
# UPDATED HEATMAP FUNCTION - NO POINTS, BETTER COLOR GRADIENTS
# ============================================================================
...
create_species_heatmap <- function(species_name, data, global_max = NULL, 
  ...
  #Creating main plot - NO SURVEY POINTS
  main_plot <- ggplot() +
    ...
    #Interpolating density surface (NO POINTS OVERLAID)
    {if(!is.null(interp_df) && nrow(interp_df) > 0) {
      geom_raster(
        data = interp_df, 
        aes(x = Longitude, y = Latitude, fill = Density), 
    ...
    #Showing density as sized/colored points ONLY for sparse species (L. alba)
    {if(is_sparse && nrow(species_pos) > 0) {
      geom_point(
        data = species_pos,
        aes(x = mean_Longitude, y = mean_Latitude, 
